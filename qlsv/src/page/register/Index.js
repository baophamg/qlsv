...
export function Index() {
    // trạng thái lưu dữ liệu danh sách đăng ký môn học từ api gởi về
    ...
    // mỗi khi page thay đổi thì getRegisters() phải chạy lại
    ...
    const handleConfirmDialog = async (currentId) => {
        try {
            const response = await axios.delete(`http://api_qlsvk99.com/api/v1/registers/${currentId}`);
            getRegisters();
    ...
    return (
        <>
            <div>
                <Helmet>
                    <title>
                        Danh sách đăng ký môn học | {process.env.REACT_APP_NAME}
                    </title>
                </Helmet>
